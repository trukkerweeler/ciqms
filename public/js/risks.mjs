...
async function openAddInputDialog() {
  ...
  if (dialog) {
    ...
    // Set INPUT_TYPE to RISK
    document.getElementById("INPUT_TYPE").value = "RISK";
...
async function saveInput(event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(`Failed to save risk: ${errorText}`);
    }
  } catch (error) {
...
async function sendEmailNotification(dataJson, nextId, myRequestDate) {
  try {
    ...
    await fetch(url + "/email", {
      ...
      if (response.ok) {
        const notifyResponse = await fetch(url + "/inputs_notify", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
async function loadInputData() {
  try {
    ...
    document.getElementById("inputTableContainer").innerHTML =
      '<p class="error">Failed to load risk data. Please refresh the page.</p>';
  }
}
...
function sortTable(columnIndex) {
  ...
  rows.sort((a, b) => {
    ...
    // Handle numeric columns (dates and numbers)

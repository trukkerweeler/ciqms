import { fetchXML, myport, parseXML, getApiUrl } from "./utils.mjs"; // Adjust the import path as necessary
const apiUrl = await getApiUrl();

// Initialize when DOM is ready
function initializeRmaWip() {
  const btnSearch = document.getElementById("btnSearch");
  const rmaNo = document.getElementById("rmaNo"); // Assuming this is the input field for Work Order No

  if (!btnSearch || !rmaNo) {
    console.warn("[rmawip.mjs] Required elements not found");
    return;
  }
  ...
  btnSearch.addEventListener("click", async function (event) {
    ...
    // Construct the URL with the RMA No and dynamic port
    const url = `${apiUrl}/rmawip/${rmaNumber}`;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeRmaWip);
} else {
  initializeRmaWip();
}

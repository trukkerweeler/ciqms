...
async function generateChart() {
  ...
  const chartCanvas = document.getElementById("ncmSubjectPieChart");
  ...
  try {
    const response = await fetch(
      `${apiUrl}/ncm/subjects-pie?period=${timePeriod}`,
    );
    if (!response.ok) throw new Error("Failed to fetch NCM subject data");
    ...
    let chartTitle = "NCM Subjects (Pareto)";
    if (timePeriod === "calendar") {
      const prevYear = new Date().getFullYear() - 1;
      chartTitle = `NCM Subjects - ${prevYear} (Pareto)`;
    } else {
      chartTitle = "NCM Subjects - Last 12 Months (Pareto)";
    }
    ...
    chartCanvas.parentElement.innerHTML = `<div style='color:red'>Failed to load NCM Subjects chart: ${err.message}</div>`;
  }
}

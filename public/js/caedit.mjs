...
button.addEventListener("click", async (event) => {
  ...
  let url = `${apiUrl}/corrective/${caidValue}`;
  // console.log(url);
      ...
      for (const key in record) {
        ...
        if (
          ...
          caDate.innerHTML =
            "Issue Date:" +
            " " +
            record[key]["CORRECTIVE_DATE"].substring(0, 10);
        } else {
          caDate.innerHTML =
            "Issue Date:" +
            " " +
...
modalsave.addEventListener("click", async (event) => {
  ...
  let url = `${apiUrl}/corrective/${cidValue}`;
  // console.log(url);
  ...
  switch (fieldname.value) {
    case "CORRECTION_TEXT":
      ...
      url = `${apiUrl}/correction/${cidValue}`;
      compositetext =
        user +
        " - " +
        mydate +
...
closecorrective.addEventListener("click", async (event) => {
  ...
  let url = `${apiUrl}/corrective/${cidValue}/close`;
...
mkfldrsave.addEventListener("click", async (event) => {
  ...
  folderfullname =
    "\\\\fs1\\Quality - Records\\10200C - Corrective Action\\" + folderfullname;

...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  url = `${apiUrl}/input/pmprogramissues`;
...
async function loadIssuesData() {
  try {
    ...
    displayIssuesTable(issuesData);
  } catch (error) {
    console.error("Error loading issues data:", error);
    document.getElementById("issuesTableContainer").innerHTML =
      '<p class="no-data-message error">Failed to load issues data. Please refresh the page.</p>';
  }
}

function displayIssuesTable(data) {
  const container = document.getElementById("issuesTableContainer");
  if (!container) return;

  if (!data || data.length === 0) {
    container.innerHTML =
      '<p class="no-data-message">No open issues found.</p>';
    return;
  }
...
function performSearch() {
  ...
  if (filteredData.length === 0) {
    document.getElementById("issuesTableContainer").innerHTML =
      '<p class="no-data-message">No issues match your search.</p>';
  } else {
    displayIssuesTable(filteredData);
  }
}
...
function exportToCSV() {
  ...
  link.setAttribute(
    "download",
    `pm-program-issues-${new Date().toISOString().split("T")[0]}.csv`,
  );

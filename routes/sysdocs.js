...
router.get("/", (req, res) => {
  try {
    ...
    connection.connect(function (err) {
      ...
      const query =
        'select DOCUMENT_ID, NAME, TYPE, STATUS, REVISION_LEVEL, ISSUE_DATE, REFERENCE, CREATE_BY, CREATE_DATE from DOCUMENTS where STATUS != "O" order by REFERENCE asc, TYPE desc, DOCUMENT_ID asc';
...
router.get("/:id", (req, res) => {
  try {
    ...
    connection.connect(function (err) {
      ...
      const query =
        "select d.*, da.CTRL_DOC, da.DIST_DOC from DOCUMENTS d left join DOCS_AVAIL da on d.DOCUMENT_ID = da.DOCUMENT_ID where d.DOCUMENT_ID = ?";
...
router.put("/:id", (req, res) => {
  try {
    ...
    connection.connect(function (err) {
      ...
      const query2 =
        "INSERT INTO DOCS_AVAIL (DOCUMENT_ID, CTRL_DOC, DIST_DOC) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE CTRL_DOC = VALUES(CTRL_DOC), DIST_DOC = VALUES(DIST_DOC)";
      connection.query(
        query2,
        [req.params.id, req.body.CTRL_DOC, req.body.DIST_DOC],
        (err, rows, fields) => {
          if (err) {

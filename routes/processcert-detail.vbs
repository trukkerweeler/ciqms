...
' ========== QUERIES ==========
sqlOps = "SELECT JO.SEQ, JO.OPERATION, JO.DESCRIPTION, JO.LMO, JO.ROUTER, JO.ROUTER_SEQ, JO.UNITS_OPEN, JO.UNITS_COMPLETE, JO.UNITS_SCRAP, JO.DATE_COMPLETED, (SELECT TOP 1 RL.PART_WC_OUTSIDE FROM ROUTER_LINE RL WHERE RL.ROUTER = JO.ROUTER AND RL.LINE_ROUTER = JO.ROUTER_SEQ) AS PART_WC_OUTSIDE, (SELECT TOP 1 OC.DESCRIPTION FROM OP_CODES OC WHERE OC.OPERATION = JO.OPERATION) AS OP_CODE_DESCRIPTION FROM JOB_OPERATIONS JO WHERE JO.JOB = '" & job & "' AND JO.SUFFIX = '" & suffix & "' AND JO.SEQ >= 100 AND JO.SEQ < 990000 AND JO.LMO NOT IN ('C', 'M') AND JO.OPERATION NOT IN ('FINALI', 'ATTACH', 'REAM', 'SANDEB', 'COUNTS', 'KITTG', 'ASSEMB', 'SHEAR', 'PUNCH', 'INSP04', 'INSP05', 'BEND', 'TUMDEB', 'DEFORM', 'PARTMK') ORDER BY JO.SEQ"
...
sqlHist = "SELECT JO.SEQ, JO.OPERATION, JO.DESCRIPTION, JO.LMO, JO.ROUTER, JO.ROUTER_SEQ, JO.UNITS_OPEN, JO.UNITS_COMPLETE, JO.UNITS_SCRAP, JO.DATE_COMPLETED, (SELECT TOP 1 RL.PART_WC_OUTSIDE FROM ROUTER_LINE RL WHERE RL.ROUTER = JO.ROUTER AND RL.LINE_ROUTER = JO.ROUTER_SEQ) AS PART_WC_OUTSIDE, (SELECT TOP 1 OC.DESCRIPTION FROM OP_CODES OC WHERE OC.OPERATION = JO.OPERATION) AS OP_CODE_DESCRIPTION FROM JOB_HIST_OPS JO WHERE JO.JOB = '" & job & "' AND JO.SUFFIX = '" & suffix & "' ORDER BY JO.SEQ"
...
Function OpRowToJSON(rs)
  ...
  Dim opCodeDesc
  opCodeDesc = NullToStr(rs("OP_CODE_DESCRIPTION"))
  
  OpRowToJSON = "{" & _
    """seq"":" & QuoteJSON(seq) & "," & _
    """operation"":" & QuoteJSON(op) & "," & _
    """description"":" & QuoteJSON(desc) & "," & _
    """opCodeDescription"":" & QuoteJSON(opCodeDesc) & "," & _
    """lmo"":" & QuoteJSON(lmo) & "," & _

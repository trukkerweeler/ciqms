...
If conn.State = 1 Then
    ...
    Dim sqlQuery
    sqlQuery = "SELECT jo.JOB, jo.SUFFIX, jo.SEQ, jo.OPERATION, jo.DESCRIPTION, jo.PART, jo.ROUTER, jo.DATE_COMPLETED, " & _
      "jd.EMPLOYEE, jd.MACHINE " & _
      "FROM JOB_OPERATIONS jo " & _
      "JOIN JOB_DETAIL jd ON jo.JOB = jd.JOB AND jo.SUFFIX = jd.SUFFIX AND jo.SEQ = jd.SEQ " & _
      "WHERE (jo.OPERATION LIKE '%D17.1%' OR jo.OPERATION LIKE '%D171%' OR jo.OPERATION LIKE '%FUSION%') " & _
      "AND jo.DATE_COMPLETED > '000000' " & _

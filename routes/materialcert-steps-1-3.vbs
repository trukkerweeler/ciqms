' globalcert-steps-1-3.vbs
' Implements Steps 1-3 of GlobalCert Chain-of-Custody extraction
' Step 1: Fetch Inventory Transactions (J52)
' Step 2: Select Transactions (user input via arguments)
' Step 3: Build first-level chain-of-custody links only
'
' Usage: cscript //Nologo globalcert-steps-1-3.vbs <JOB> <SUFFIX> [selectedTransactionIndices]
' Example: cscript //Nologo globalcert-steps-1-3.vbs 122166 001 0,1,3
...
If WScript.Arguments.Count > 0 Then
  ...
  End If
Else
  WScript.Echo "Usage: globalcert-steps-1-3.vbs <JOB> <SUFFIX> [selectedTransactionIndices]"
  WScript.Quit 1
End If
...
' ============================================================================
Dim step1Results
Set step1Results = CreateObject("Scripting.Dictionary")
Dim j52Transactions
...
' ============================================================================
' STEP 2: Select Transactions (via command-line argument)
' ============================================================================
Dim selectedIndices
...
For i = 1 To j52Transactions.Count
  ...
  If selectedIndices.Exists(currentTxn("index") - 1) Or selectedIndices.Exists(CStr(currentTxn("index") - 1)) Then
    ...
    ' Create CoC link object
    Dim cocLink
    Set cocLink = CreateObject("Scripting.Dictionary")
    cocLink("parentJob") = job
    cocLink("parentSuffix") = suffix
    cocLink("j52Transaction") = currentTxn
    
    ' Load child job header (JOB_HEADER) using SERIAL_NUMBER
    Dim sqlChildHeader
...
Dim output
output = "{"
output = output & """step1_j52_transactions"": " & DictionaryToJSON(j52Transactions) & ", "

'C:\Windows\SysWOW64\cscript.exe C:\Users\TimK\Documents\RMA\routes\certfwld.vbs
...
If conn.State = 1 Then
    ...
    sqlQuery = "SELECT TOP 1 jo.JOB, jo.SUFFIX, jo.SEQ, jo.DESCRIPTION, jo.ROUTER, jo.ROUTER_SEQ, " & _
           ...
           "AND LOWER(jo.DESCRIPTION) like '%fusion%' " & _
           "ORDER BY jo.SEQ ASC"
    
    ' MsgBox "SQL Query: " & sqlQuery, vbInformation, "certfwld.vbs 52"
    ...
    Else
        ...
        Dim completedDate
        sqlQuery = "SELECT DATE_COMPLETED FROM JOB_OPERATIONS WHERE JOB = '" & woBase & "' AND SUFFIX = '" & woChild & "' AND (lower(DESCRIPTION) like '%fusion%' or lower(DESCRIPTION) like '%d171c%')"
        rs.Close
        rs.Open sqlQuery, conn, 3, 1 ' 3 = adOpenStatic, 1 = adLockReadOnly
        If Not rs.EOF Then
